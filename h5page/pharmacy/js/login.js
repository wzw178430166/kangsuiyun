...
//手机号码输入框
var inputPhone=document.getElementsByName('username')[0];
//alert(inputPhone);
//密码输入框
        ...
        $.post('/index.php?m=member&c=index&a=login&ajax=1',$('#loginForm').serialize(),function(data){
			...
			if (data.status == 1) {
				...
				setTimeout(function(){
					...
					window.location.href='/h5page/pharmacy/'
					// 	//window.location.href = data.forward ? data.forward :'/h5page/pharmacy/';
				},500);
...
//获取手机号码输入框
var noPhoneNum=document.getElementsByName('username')[1];
