...
class index extends admin {
	...
	public function login() {
		if(isset($_GET['dosubmit'])) {
			...
			//
			$_POST['password'] = substr( base64_decode($_POST['password']),4);
			//
	...
	public function public_main() {
		...
		$sysinfo['mysqlv'] = mysqli_get_server_info($db->get_link());
	...
	public function public_snda_status() {
		...
		if(!strstr(pc_base::load_config('snda','snda_status'), '|')) {
			...
			$uuid = $uuid_arr['uuid'];
			$snda_check_url = "http://open.sdo.com/drcms?cmsid=".$uuid."&sitedomain=".$_SERVER['SERVER_NAME'];
