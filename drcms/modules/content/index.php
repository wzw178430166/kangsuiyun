...
class index {
	...
	function __construct() {
		...
		$this->is_app = isset($_GET['app'])? 1: 0;
		$this->is_web = isset($_GET['web'])? 1: 0;
		$this->is_mobile = param::get_cookie('_is_mobile');
		if($_GET['wap'] || $this->is_app){
			$this->is_mobile = 1;	
		}elseif($this->is_web){
			$this->is_mobile = 0;
		...
		param::set_cookie('_is_mobile', $this->is_mobile);
	}
	//首页
	public function init() {
		/*if(1>3&&$this->is_mobile || isset($_GET['app'])){
			header('Location:index.php?m=app');
			exit();	
		}*/
		//********
		$site=require drcms_PATH."caches/configs/route.php";
		if(in_array($_REQUEST['a'],array('login','register','logout'))){
			$siteid="1";
		}else{
			$siteid=$site[$_SERVER['HTTP_HOST']]['data']['siteid'];
		}
...
//		}
		...
		//*******
		$sitelist_=$sitelist[$siteid];
		
		if($sitelist_['ishtml']==1 && $_REQUEST['php']!=1){
			$dirname=$sitelist_['dirname']?  "html".DIRECTORY_SEPARATOR.$sitelist_['dirname'].DIRECTORY_SEPARATOR : "" ;
			include (drcms_PATH.$dirname."index.html");
		}else{
			include template('content','index',$default_style);
		}
	}
	...
	public function lists() {
		...
		if($type==0) {
			...
			//echo template('content',$template);exit;
			include template('content',$template);
		} else {
		//单网页
